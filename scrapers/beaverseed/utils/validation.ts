/**
 * Validation utilities for Beaver Seed scraper
 * 
...
export function validateSourceContext(
  ...
  if (!sourceContext) {
    const errorMsg = '[Beaver Seed] sourceContext is required but was undefined';
  ...
  if (!sourceContext.scrapingSourceUrl || sourceContext.scrapingSourceUrl.trim() === '') {
    const errorMsg = '[Beaver Seed] scrapingSourceUrl is required in sourceContext';
    apiLogger.logError(errorMsg, new Error(errorMsg), {
      sourceContext,
      siteConfig: siteConfig.name
    });
  ...
  try {
    ...
    const errorMsg = `[Beaver Seed] Invalid scrapingSourceUrl format: ${sourceContext.scrapingSourceUrl}`;
  ...
  if (!sourceContext.sourceName || sourceContext.sourceName.trim() === '') {
    apiLogger.warn('[Beaver Seed] sourceName missing, using siteConfig.name', {
  ...
  if (!sourceContext.dbMaxPage || sourceContext.dbMaxPage < 1) {
    apiLogger.warn('[Beaver Seed] Invalid dbMaxPage, using default: 100', {
...
export function validateScrapingMode(
  ...
  if (isTestMode && effectiveStartPage > effectiveEndPage!) {
    apiLogger.warn('[Beaver Seed] Invalid page range: startPage > endPage', {
...
export function logScraperInitialization(
  ...
  apiLogger.debug('[Beaver Seed] Starting with configuration', {
