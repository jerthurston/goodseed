 ...
 */
export function getCategoryUrl(baseUrl: string, categorySlug: string, page: number = 1): string {
    if (page === 1) {
        return `${baseUrl}/${categorySlug}.html`;
    }
    return `${baseUrl}/${categorySlug}.html?p=${page}`;
}
