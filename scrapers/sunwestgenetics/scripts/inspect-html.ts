...
async function fetchAndInspectHTML(url: string): Promise<void> {
    ...
    try {
        ...
        if ($('body').hasClass('woocommerce') || $('.woocommerce').length > 0) {
            ...
            console.log('❌ WooCommerce not detected');
        }
        ...
        if ($('[data-shopify]').length > 0 || html.includes('Shopify')) {
            ...
            console.log('✅ Magento detected');
        } else {
            console.log('❓ Platform not clearly detected');
        }
...
async function main() {
    console.log('🕵️ SunWest Genetics HTML Inspector');
