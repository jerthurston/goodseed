/**
 * 🔍 SIMPLE TRUE NORTH SEED BANK - extractProductUrlsFromCatLink TEST
 * 
 * Simplified test without Prisma dependencies
 */
...
async function testExtractProductUrlsFromCatLink() {
    ...
    // Initialize without ScraperFactory
    ...
    console.log('🤖 Getting robots.txt rules...');
    const robotsRules = await politeCrawler.parseRobots(baseUrl);
    console.log(`✅ Robots rules loaded - crawl delay: ${robotsRules.crawlDelay}ms`);
...
async function analyzePageStructure(url: string) {
    ...
    const crawler = new CheerioCrawler({
        ...
        requestHandler: async ({ $, request }) => {
            ...
            const otherSelectors = [
                ...
                { name: 'WooCommerce products', selector: '.woocommerce-LoopProduct-link' }
            ];
            ...
            console.log('\n📋 SAMPLE LINKS (first 15 for analysis):');
