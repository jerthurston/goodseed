/**
 * Database Service for Royal Queen Seeds Category Scraper
 * 
 ...
 * 
 * Based on Seed Supreme's category-db-service.ts
 * Adapted for Royal Queen Seeds with:
 * - Default pack size: 5 seeds (vs Seed Supreme's 4)
 * - Effects field instead of flavor
 * - Rating and review count from product cards
 * 
 * @usage
 * ```typescript
...
const SELLER_NAME = 'Royal Queen Seeds';
const SELLER_URL = 'https://www.royalqueenseeds.com';

export class RoyalQueenSeedsCategoryDbService {
    ...
    async saveProductsToCategory(
        ...
        for (const product of products) {
            try {
                ...
                // Parse THC level (e.g., "25%" -> 25)
                const thcNum = product.thcLevel
                    ? parseFloat(product.thcLevel.replace('%', '').trim())
                    : null;
                ...
                const savedProduct = await this.prisma.seedProduct.upsert({
                    ...
                    update: {
                        name: product.name,
                        url: product.url,
                        description: product.effects || null,
                        basePrice: product.basePriceNum || 0,
                        packSize: product.packSize || 5, // RQS default: 5 seeds
                        pricePerSeed: product.pricePerSeed || (product.basePriceNum ? product.basePriceNum / (product.packSize || 5) : 0),
                        stockStatus: this.mapStockStatus(product.stockStatus),
                        cannabisType: null, // Not available in product cards
                        variety: product.genetics || null,
                        thcMin: thcNum,
                        thcMax: thcNum,
